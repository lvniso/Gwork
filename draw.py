    ...
    for a in range(3,ans+1):
        for b in range(1,ans+1):
            ...
            for c in range(1, a-1):
                for d in range(1, b):
                    for e in range(1,b):
                        if (a*b-c*max(d,e)-(a-c)*(b-min(d,e))/2)*x*x >= S*(1-rate) and (a*b-c*max(d,e)-(a-c)*(b-min(d,e))/2)*x*x <= S*(1+rate):
                        ...
                        elif (a*b-c*max(d,e)-(a-c)*(b-min(d,e))/2)*x*x > S*(1+rate):
                            continue
    return result
